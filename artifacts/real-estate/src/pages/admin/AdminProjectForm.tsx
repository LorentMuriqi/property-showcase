      ...
      <div className="sticky top-0 z-40 bg-background/80 backdrop-blur-xl border-b border-white/5 p-4 md:p-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-xs uppercase tracking-widest mt-1">Listing Configuration</p>
          </div>
        </div>
        <button 
        ...
        >
          <Save size={18} /> {isEditing ? "Save Changes" : "Publish Listing"}
        </button>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 mt-8">
        <form className="space-y-8">
          ...
          <div className="glass-panel p-6 md:p-8 rounded-2xl space-y-6">
            <h2 className="font-display text-xl text-primary border-b border-white/10 pb-4">Essential Details</h2>
            
            <Input label="Property Title *" {...register("title", { required: true })} placeholder="E.g., The Bel Air Estate" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-xs font-medium text-white/70 uppercase tracking-wider">Status *</label>
                <select 
                  {...register("status")}
            ...
            <div className="space-y-2">
              <label className="text-xs font-medium text-white/70 uppercase tracking-wider">Description</label>
              <textarea 
          ...
          {/* Specs */}
          <div className="glass-panel p-6 md:p-8 rounded-2xl space-y-6">
            <h2 className="font-display text-xl text-primary border-b border-white/10 pb-4">Specifications</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <Input label="Price" type="number" {...register("price")} />
              <Input label="Currency" {...register("currency")} placeholder="USD" />
              <Input label="Area (m²)" type="number" {...register("areaM2")} />
              <Input label="Property Type" {...register("propertyType")} placeholder="Villa, Penthouse..." />
              <Input label="Bedrooms" type="number" {...register("bedrooms")} />
              <Input label="Bathrooms" type="number" {...register("bathrooms")} />
              <Input label="Living Rooms" type="number" {...register("livingRooms")} />
              <Input label="Floors" type="number" {...register("floors")} />
              <Input label="Year Built" type="number" {...register("yearBuilt")} />
            </div>
          </div>
          ...
          <div className="glass-panel p-6 md:p-8 rounded-2xl space-y-6 border-l-4 border-l-primary">
            <h2 className="font-display text-xl text-primary border-b border-white/10 pb-4 flex items-center gap-2">
              Virtual Experience (360° Tour)
            </h2>
            <p className="text-sm text-muted-foreground">Provide either a direct link to the tour OR an embed code (iframe) from providers like Kuula, Matterport, etc.</p>
            
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-xs font-medium text-white/70 uppercase tracking-wider flex items-center gap-2"><LinkIcon size={14}/> Tour URL</label>
                <input 
                  type="url"
              ...
              <div className="flex items-center gap-4 text-white/30 uppercase text-xs font-bold before:flex-1 before:h-px before:bg-white/10 after:flex-1 after:h-px after:bg-white/10">OR</div>
              <div className="space-y-2">
                <label className="text-xs font-medium text-white/70 uppercase tracking-wider flex items-center gap-2"><Code size={14}/> Embed Code (HTML)</label>
                <textarea 
                  rows={4}
          ...
          <div className="glass-panel p-6 md:p-8 rounded-2xl space-y-6">
            ...
            <div className="space-y-4">
              ...
              {imageFields.length === 0 && (
                <p className="text-center text-muted-foreground py-4">No images added. Click "Add Image" above.</p>
              )}
            </div>
          </div>
